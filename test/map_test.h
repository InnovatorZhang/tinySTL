...
// pair 的宏定义
#define PAIR    tinySTL::pair<int, int>
...
#define MAP_VALUE(fun) do { \
    ...
    std::cout << " " << str << " : <" << it.first << "," << it.second << ">\n"; \
            ...
            void map_test() {
                ...
                tinySTL::map<int, int> m1;
                tinySTL::map<int, int, tinySTL::greater<int>> m2;
                tinySTL::map<int, int> m3(v.begin(), v.end());
                tinySTL::map<int, int> m4(v.begin(), v.end());
                tinySTL::map<int, int> m5(m3);
                tinySTL::map<int, int> m6(std::move(m3));
                tinySTL::map<int, int> m7;
                m7 = m4;
                tinySTL::map<int, int> m8;
                m8 = std::move(m4);
                tinySTL::map<int, int> m9{PAIR(1, 1), PAIR(3, 2), PAIR(2, 3)};
                tinySTL::map<int, int> m10;
            ...
            void multimap_test() {
                ...
                for (int i = 0; i < 5; ++i)
                    v.push_back(PAIR(i, i));
                tinySTL::multimap<int, int> m1;
                tinySTL::multimap<int, int, tinySTL::greater<int>> m2;
                tinySTL::multimap<int, int> m3(v.begin(), v.end());
                tinySTL::multimap<int, int> m4(v.begin(), v.end());
                tinySTL::multimap<int, int> m5(m3);
                tinySTL::multimap<int, int> m6(std::move(m3));
                tinySTL::multimap<int, int> m7;
                m7 = m4;
                tinySTL::multimap<int, int> m8;
                m8 = std::move(m4);
                tinySTL::multimap<int, int> m9{PAIR(1, 1), PAIR(3, 2), PAIR(2, 3)};
                tinySTL::multimap<int, int> m10;
                m10 = {PAIR(1, 1), PAIR(3, 2), PAIR(2, 3)};
                ...
                MAP_FUN_AFTER(m1, m1.erase(m1.begin(), m1.end()));
                for (int i = 0; i < 5; ++i) {
                    MAP_FUN_AFTER(m1, m1.insert(tinySTL::make_pair(i, i)));
                }
                MAP_FUN_AFTER(m1, m1.insert(v.begin(), v.end()));
